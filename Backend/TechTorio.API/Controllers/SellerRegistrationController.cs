...
public class SellerRegistrationController : ApiControllerBase
{
    ...
    [HttpPost("apply-user")]
    public async Task<IActionResult> ApplyForUserRole(TechTorio.Application.Features.UserManagement.Commands.ApplyForUserRole.ApplyForUserRoleCommand command)
    {
        var result = await Mediator.Send(command);
