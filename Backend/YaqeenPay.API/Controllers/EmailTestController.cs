    ...
    public async Task<IActionResult> SendTestEmail([FromBody] TestEmailRequest request)
    {
        try
        {
            await _emailService.SendEmailAsync(
                request.ToEmail,
                request.Subject ?? "Test Email from YaqeenPay",
                request.HtmlBody ?? "<h1>Test Email</h1><p>This is a test email from YaqeenPay mail system.</p>",
                request.PlainTextBody ?? "Test Email - This is a test email from YaqeenPay mail system.");
    ...
    public async Task<IActionResult> SendPasswordReset([FromBody] PasswordResetEmailRequest request)
    {
        try
        {
            await _emailService.SendPasswordResetEmailAsync(
                request.ToEmail,
                request.ResetLink ?? "https://techtorio.online/yaqeenpay/reset-password?token=test123",
