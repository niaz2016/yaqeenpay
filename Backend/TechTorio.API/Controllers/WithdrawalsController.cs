...
using TechTorio.Application.Features.Withdrawals.Commands.RequestWithdrawal;
using TechTorio.Application.Features.Withdrawals.Queries.GetWithdrawals;
...
public class WithdrawalsController : ApiControllerBase
{
    ...
    public async Task<IActionResult> GetWithdrawalById(string withdrawalId)
    {
        ...
        var result = await Mediator.Send(new TechTorio.Application.Features.Withdrawals.Queries.GetWithdrawalById.GetWithdrawalByIdQuery(id));
        if (result == null)
            return NotFound();
        return Ok(result);
    }
    ...
    public async Task<IActionResult> CancelWithdrawal(string withdrawalId)
    {
        ...
        var result = await Mediator.Send(new TechTorio.Application.Features.Withdrawals.Commands.CancelWithdrawal.CancelWithdrawalCommand(id));
        if (!result)
